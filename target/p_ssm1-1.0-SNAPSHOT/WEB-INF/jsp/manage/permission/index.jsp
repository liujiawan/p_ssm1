...
<html>
	...
	<body style="margin:0px; height:82%;">
		   ...
		   <div class="modal-dialog">
		      <div class="modal-content">
		         ...
		         <div class="modal-footer">
		            ...
		            <button type="button" id="buttonEdit" class="btn btn-primary" onclick="edit('<%=basePath%>Manage/Permission/edit.do?date='+new Date())">
		               	确认
		            </button>
		         </div>
		      </div><!-- /.modal-content -->
		...
		<div class="panel-body" style="padding-bottom: 0px;width:80%;float:right">
			<div id="toolbar" class="btn-group" style="width:35%;float:left">
				...
				<button id="btn_delete" type="button" class="btn btn-default" onclick="del('<%=basePath%>Manage/Permission/delete.do?date='+new Date())">
    ...
    function addHoverDom(treeId, treeNode) {
	        ...
	        if (btn) btn.bind("click", function(){	
	        	//将新节点添加到数据库中  
	            var name='NewNode';  
	            $.post('<%=basePath%>Manage/ZtreeNode/add.do?pid='+treeNode.id+'&name='+name+'&type=2',function (data) {  
	...
	function initTreeNode(){
		$.ajax({
			url:'<%=basePath%>Manage/ZtreeNode/list.do?type=2',
			type:'GET',
			async:false,
	...
	function initTable() {  
        ...
        $("#table").bootstrapTable({  
            ...
            url: "<%=basePath%>Manage/Permission/list.do?date=" + new Date(), //获取数据的Servlet地址  
            striped: true,  //表格显示条纹  
            ...
            queryParams: function queryParams(params) {
              ...
              var param = {    
                  ...
                  //queryString : $("#txt_search_Permission_name").val(),
                  treeNodeId : nodeId
              };    
              return param;                   
            },
            columns : [ {
                    ...
                    formatter:function(value,row,index){
                    	...
                    	return " <div class=\"fa-hover col-md-7 col-sm-4\"><i class=\"fa fa-close\"></i>&nbsp;&nbsp;不可用</a></div><div style=\"margin-right:5%;float:right\"><button class='layui-btn layui-btn-mini' onclick='showPermissionTree("+value+")'> <i class=\"fa fa-asterisk\"></i>&nbsp;&nbsp;设置</button></div>";
                    }
				}],  
